...
def test_irc_msg():
    ...
    irc_msg = TwitchIRCMsg(raw_irc_msg)
    irc_msg_tags = {
...
def test_parse_raw_tags():
    irc_msg = TwitchIRCMsg(r'@no-value-tag;key=value;key2=escaped\svalue\\\: COMMAND')
    assert irc_msg == {'no-value-tag': None, 'key': 'value', 'key2': r'escaped value\;'}
    # None
    irc_msg = TwitchIRCMsg('COMMAND')
    assert irc_msg == {}


def test_parse_prefix():
    # servername
    irc_msg = TwitchIRCMsg(r':server.name.tv COMMAND')
    ...
    # full
    irc_msg = TwitchIRCMsg(r':nickname!user@host COMMAND')
    assert irc_msg.servername is None
    assert irc_msg.nickname == 'nickname'
    assert irc_msg.user == 'user'
    assert irc_msg.host == 'host'
    # no user
    irc_msg = TwitchIRCMsg(r':nickname@host COMMAND')
    assert irc_msg.servername is None
    assert irc_msg.nickname == 'nickname'
    assert irc_msg.user is None
    assert irc_msg.host == 'host'
    # no user, no host
    irc_msg = TwitchIRCMsg(r':nickname COMMAND')
    assert irc_msg.servername is None
    assert irc_msg.nickname == 'nickname'
    assert irc_msg.user is None
    assert irc_msg.host is None
...
def test_parse_raw_params():
    # middle
    irc_msg = TwitchIRCMsg('COMMAND middle')
    assert irc_msg.middles == ('middle',)
    assert irc_msg.trailing is None
    # middle, trailing (':')
    irc_msg = TwitchIRCMsg('COMMAND middle :trailing')
    assert irc_msg.middles == ('middle',)
    assert irc_msg.trailing == 'trailing'
    # same and trailing contains separators
    irc_msg = TwitchIRCMsg('COMMAND middle :trai :ling')
    assert irc_msg.middles == ('middle',)
    assert irc_msg.trailing == 'trai :ling'
    # middle, trailing (no ':')
    irc_msg = TwitchIRCMsg('COMMAND ' + 'middle ' * 14 + 'trailing')
    assert irc_msg.middles == ('middle',) * 14
    assert irc_msg.trailing == 'trailing'
    # same and trailing contains separators
    irc_msg = TwitchIRCMsg('COMMAND ' + 'middle ' * 14 + 'trai :ling')
    assert irc_msg.middles == ('middle',) * 14
    assert irc_msg.trailing == 'trai :ling'
    # trailing
    irc_msg = TwitchIRCMsg('COMMAND :trailing')
    ...
    assert irc_msg.trailing is None
    # channel
    irc_msg = TwitchIRCMsg('COMMAND #channel_login middle last')
    assert irc_msg.channel == 'channel_login'
    irc_msg = TwitchIRCMsg('COMMAND first #channel_login last')
    assert irc_msg.channel == 'channel_login'
    irc_msg = TwitchIRCMsg('COMMAND first middle #channel_login')
    assert irc_msg.channel == 'channel_login'
    irc_msg = TwitchIRCMsg('COMMAND first middle #channel_login :trai :ling')
    assert irc_msg.channel == 'channel_login'
    irc_msg = TwitchIRCMsg('COMMAND first middle channel_login :trai :ling')
...
def test_join_tags():
    irc_msg = TwitchIRCMsg.create_empty()
    irc_msg = {'one-tag': 'one-value'}
    assert irc_msg._join_tags() == 'one-tag=one-value'
    irc_msg = {'one-no-tag-value': None}
    assert irc_msg._join_tags() == 'one-no-tag-value'
    irc_msg = {'key': 'value', 'no-value-tag': None, 'key2': 'value2'}
    assert irc_msg._join_tags() == 'key=value;no-value-tag;key2=value2'


def test_eq():
    # same positions tags
    irc_msg = TwitchIRCMsg('@key=value;no-value-tag :username COMMAND')
    new_msg = TwitchIRCMsg('@key=value;no-value-tag :username COMMAND')
    assert irc_msg == new_msg
    # different positions tags
    irc_msg = TwitchIRCMsg('@key=value;no-value-tag :username COMMAND')
    new_msg = TwitchIRCMsg('@no-value-tag;key=value :username COMMAND')
    assert irc_msg == new_msg
    # same positions middles
    irc_msg = TwitchIRCMsg('@key=value;no-value-tag :username COMMAND param #target')
    new_msg = TwitchIRCMsg('@no-value-tag;key=value :username COMMAND param #target')
    assert irc_msg == new_msg
    # different positions middles
    irc_msg = TwitchIRCMsg('@key=value;no-value-tag :username COMMAND param #target :trai :ling')
    new_msg = TwitchIRCMsg('@no-value-tag;key=value :username COMMAND #target param :trai :ling')
    assert irc_msg == new_msg
    # others
    assert TwitchIRCMsg('COMMAND') == TwitchIRCMsg('COMMAND')
    assert TwitchIRCMsg('COMMAND') != TwitchIRCMsg('ERROR')


def test_str_repr():
    assert str(TwitchIRCMsg('COMMAND')) == 'COMMAND'
    assert repr(TwitchIRCMsg('COMMAND')) == 'COMMAND'
    raw_irc_msg = '@key=value;no-value-tag :username COMMAND param #target :trai :ling'
    assert str(TwitchIRCMsg(raw_irc_msg)) == raw_irc_msg
    raw_irc_msg = '@badge-info=subscriber/1;badges=subscriber/0;client-nonce=3f58e4f3107d580b8a29626738823a5c;color=#FF69B4;display-name=fernandx_z;emotes=;flags=;id=aa3b5987-1929-414c-bc55-10f9e6c1723e;mod=0;reply-parent-display-name=MaYidRaMaS;reply-parent-msg-body=axozerTem\\sato\\saxozerPium\\saxozerPium_HF;reply-parent-msg-id=2f06b2b8-d33d-4e65-a0c4-82d1894c7b63;reply-parent-user-id=612074199;reply-parent-user-login=mayidramas;room-id=133528221;subscriber=1;tmi-sent-ts=1622471612333;turbo=0;user-id=602696060;user-type= :fernandx_z!fernandx_z@fernandx_z.tmi.twitch.tv PRIVMSG #axozer :@MaYidRaMaS PERO JAJSJAJSJASJASJA'
    assert str(TwitchIRCMsg(raw_irc_msg)) == raw_irc_msg
